...
class GenerateQuestionsRequest(BaseModel):
    ...
    def validate_model(cls, v):
        allowed_models = ["llama3-70b", "deepseek-r1", "llama3-8b", "gemma2-9b", "openai-oss-20b", "gemini-2.0-flash", "gemini-2.5-flash", "moonshot-k2"]
...
class GenerateWholePaperRequest(BaseModel):
    ...
    @validator('model')
    def validate_model(cls, v):
        allowed_models = ["llama3-70b", "deepseek-r1", "gemma2-9b", "openai-oss-20b", "gemini-2.5-pro", "gemini-2.5-flash", "moonshot-k2"]
        if v not in allowed_models:
            raise ValueError(f'Model must be one of {allowed_models}')
        return v
