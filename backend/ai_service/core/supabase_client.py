...
def get_supabase_service() -> SupabaseService:
    ...
    def get_analytics_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get total questions generated by the user
            ...
            feedback_res = client.table('question_feedback').select('rating', count='exact').execute()

            feedback_summary = {}
            if feedback_res.data:
                # This is a bit of a hack, but it works for now.
                # A better way would be to use a database function.
                ratings = [item['rating'] for item in feedback_res.data]
                feedback_summary = {
                    'good': ratings.count(5) + ratings.count(4),
                    'bad': ratings.count(1) + ratings.count(2),
                }
