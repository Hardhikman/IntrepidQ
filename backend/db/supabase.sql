...
-- Guest generations tracking for IP-based rate limiting
...
-- guest_generations policies (service key only)
...
-- 9) Guest IP Cleanup Function and Cron Job ---------------

-- Enable pg_cron extension (needs to be run by superuser)
-- Run this manually in Supabase SQL editor as superuser:
-- create extension if not exists pg_cron;

-- Function to clean up old guest generation records
...
-- Create the cron job to run cleanup every 2 days at 2 AM UTC
-- Note: This requires pg_cron extension and superuser privileges
-- IMPORTANT: Run these commands MANUALLY in Supabase SQL Editor after applying this schema:

-- Step 1: Enable pg_cron extension (run as superuser)
-- CREATE EXTENSION IF NOT EXISTS pg_cron;

-- Step 2: Schedule the cleanup job (run after enabling pg_cron)
...
-- Alternative: Manual cleanup function that can be called via API
