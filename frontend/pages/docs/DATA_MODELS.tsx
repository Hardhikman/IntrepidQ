...
export async function getStaticProps() {
  ...
  const fullPath = join(docsDirectory, 'DATA_MODELS.md')
  
  try {
    const fileContents = await fs.readFile(fullPath, 'utf8')
    
    // Enhanced frontmatter extraction
    let title = 'Data Models'
    let description = ''
...
export default function DATA_MODELSDoc({ title, description, content, docId }: { title: string, description: string, content: string, docId: string }) {
