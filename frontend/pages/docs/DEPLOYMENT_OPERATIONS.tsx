...
export async function getStaticProps() {
  ...
  const fullPath = join(docsDirectory, 'DEPLOYMENT_OPERATIONS.md')
  
  try {
    const fileContents = await fs.readFile(fullPath, 'utf8')
    
    // Enhanced frontmatter extraction
    let title = 'Deployment and Operations'
    let description = ''
...
export default function DeploymentOperationsDoc({ title, description, content, docId }: { title: string, description: string, content: string, docId: string }) {
