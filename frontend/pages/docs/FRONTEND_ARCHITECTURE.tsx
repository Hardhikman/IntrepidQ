...
export async function getStaticProps() {
  ...
  const fullPath = join(docsDirectory, 'FRONTEND_ARCHITECTURE.md')
  
  try {
    const fileContents = await fs.readFile(fullPath, 'utf8')
    
    // Enhanced frontmatter extraction
    let title = 'Frontend Architecture'
    let description = ''
...
export default function FrontendArchitectureDoc({ title, description, content, docId }: { title: string, description: string, content: string, docId: string }) {
