...
- **Supabase pgvector**: Efficient similarity search
...
The system uses Qwen on Groq:
- **Groq**: Primary provider with Qwen3-32b model
...
### News Retrieval Migration
The system has transitioned from Tavily to DuckDuckGo Search (DDGS):
- **Cost Efficiency**: Moved to a completely free, no-API-key-required solution.
- **Reliability**: Implemented intelligent fallback logic (News Headlines → General Web Search).
- **Rate Handling**: Added exponential backoff to handle search throttling gracefully.