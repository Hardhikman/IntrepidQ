...
export async function getStaticProps() {
  ...
  const fullPath = join(docsDirectory, 'TECHNOLOGY_STACK.md')
  
  try {
    const fileContents = await fs.readFile(fullPath, 'utf8')
    
    // Enhanced frontmatter extraction
    let title = 'Technology Stack'
    let description = ''
...
export default function TechnologyStackDoc({ title, description, content, docId }: { title: string, description: string, content: string, docId: string }) {
